...
public class LaunchPrecheck {
    public static func checkJava(_ instance: MinecraftInstance, _ options: LaunchOptions) -> Result<Void, JavaCheckError> {
        ...
        let suitableJava = MinecraftInstance.findSuitableJava(instance.version, minJavaVersion: minJavaVersion, manifest: instance.manifest)

        // Java 扫描结果判空（排除 /usr/bin/java 这种系统 stub）
        let realJVMMachines = DataManager.shared.javaVirtualMachines
            .filter { $0.executableURL.path != "/usr/bin/java" }
        if realJVMMachines.isEmpty {
        ...
        if let suitableJava {
            ...
            if Architecture.system == .x64 && javaArchitecture == .arm64 {
                ...
                warn("[launchPrecheck] 正在使用 x64 Java (Rosetta)")
                // 不阻断启动，交由上层决定
            }
        } else {
            err("[launchPrecheck] 无可用 Java。最低版本: \(minJavaVersion)")
            return .failure(.noUsableJava(minVersion: minJavaVersion))
        }
