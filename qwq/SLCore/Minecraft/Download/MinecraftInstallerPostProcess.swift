...
extension MinecraftInstaller {
    ...
    /// 而 `slLaunchInternal` 的执行顺序是「先启动前补全（`LaunchFix.perform` → `ensureNatives`
    ...
    static func finalWork(_ task: MinecraftInstallTask) {
        ...
        // 初始化实例配置：**仅当 `.SL.json` 尚不存在时**才新建实例并落默认配置。
        // ⚠️ 覆盖安装时该文件已存在（内含用户自定的 Java / 内存等），无条件 `saveConfig()` 会把默认
        // 配置盖回去 —— 一次重装或补全就把用户的实例设置清零。
