...
- 下载详情页毛玻璃化：左侧版本分类栏与右侧内容区（ModDetailView）整体包成启动器风格的圆角矩形毛玻璃卡片（`regularMaterial` + 圆角 20 + 阴影，四周留 8pt 间隙），与主界面视觉统一
- 下载按钮改为胶囊圆形样式（参照 PCL.Mac `RoundedButton`）：强调色胶囊 + 白字「开始下载」+ 下载图标 + 光晕阴影，保留原有按压回弹与圆形毛玻璃指示按钮弹入动画；功能与交互全部保持不变
