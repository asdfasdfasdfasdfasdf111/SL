...
修复启动页头像延迟约 0.x 秒才出现：原实现在 SwiftUI body 内同步 `Data(contentsOf:)` 读皮肤贴图且依赖 onAppear 异步加载两个图层裁剪，首帧头像为空、等后台加载完才显示。现视图创建时同步预载持久化头像（无持久化头像时回退内置皮肤，个位数毫秒级小图），首帧直接渲染不再空白；JAR 提取 / 皮肤缓存结果在后台刷新后经 onChange 更新内存缓存，body 内不再每次重绘都重复解图
